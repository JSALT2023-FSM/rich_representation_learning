...
class EmoIdBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        transcript = self.asr_model.transcribe_batch(wavs.to(self.device), lens.to(self.device))
        tokens_list = self.tokenizer.encode_as_ids(transcript[0])
        print(tokens_list)
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.provides(
        "transcript", "tokens_list", "tokens"
    )
    def text_pipeline(transcript):
        yield transcript
        tokens_list = tokenizer.encode_as_ids(transcript)
        yield tokens_list
        tokens = torch.LongTensor([hparams["bos_index"]] + (tokens_list))
        yield tokens
    ...
    sb.dataio.dataset.set_output_keys(
        datasets,
        ["id", "sig", "scenario", "scenario_encoded", "action", "action_encoded", "transcript", "tokens_list", "tokens"],
    )
    ...
    return {"train": datasets[0], "valid" : datasets[1], "test" : datasets[2]} , tokenizer
...
if __name__ == "__main__":
    ...
    from asr_prepare import prepare_SLURP  # noqa
    ...
    emo_id_brain.tokenizer = tokenizer
