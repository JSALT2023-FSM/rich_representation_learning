...
class EmoIdBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        ctc = self.asr_model.mods.encoder(wavs, lens)
        # last dim will be used for AdaptativeAVG pool
        outputs = self.modules.slu_enc(ctc)
    ...
    def init_optimizers(self):
        ...
        self.wav2vec2_optimizer = self.hparams.wav2vec2_opt_class(self.asr_model.parameters())
        self.optimizer = self.hparams.opt_class(self.hparams.model.parameters())

        if self.checkpointer is not None:
            self.checkpointer.add_recoverable("modelopt", self.optimizer)
            self.checkpointer.add_recoverable("asr_model", self.asr_model)
...
def dataio_prep(hparams):
    ...
    sb.dataio.dataset.set_output_keys(
        datasets,
        ["id", "sig", "scenario", "scenario_encoded", "action", "action_encoded"],
    )
