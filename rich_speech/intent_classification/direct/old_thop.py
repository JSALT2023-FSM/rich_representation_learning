...
class EmoIdBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        if thop :
            macs, par = profile(hf_model, inputs=(wavs.to(self.device), ), verbose =False)
        ...
        if thop :  
            second_macs, par = profile(self.modules.embedding_model, inputs = (layer_0,), verbose = False)
        embeddings = self.modules.embedding_model(layer_0, lens)
        if thop : 
            third_macs , par = profile(self.modules.classifier, inputs = (embeddings, ), verbose = False)
            all_macs.append(macs+second_macs+third_macs)
