...
def dataio_prep(hparams):
    ...
    def text_pipeline(wav):
        idf = wav.split("/")[-1]
        asr_transcripts = transcriptions[idf][0:n_considered]
        asr_transcript = " ".join(asr_transcripts)
        yield asr_transcript
        tokens_list = simple_tokenizer(asr_transcript)
        yield tokens_list
...
if __name__ == "__main__":
    ...
    n_considered  = 5
    #Read the topn file :
    file_topn = "/gpfsstore/rech/nou/uzn19yk/JSALT/data/out_hyps_bmsize128.txt"
    with open(file_topn, "r") as ft1 : 
        ...
        for line in lines : 
            if line[:2]=="id" : 
                ...
                transcriptions[idf]=[]
                counter = 0
                transcriptions_tokenized[idf]= []
            else : 
                sentence = line.split("/")[1]
                if counter ==n_considered :
                    continue
