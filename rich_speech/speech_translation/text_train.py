...
class ST(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        sentence, _ = batch.sentence_tokens
        emb_sentence = self.modules.output_emb(sentence)
        # transformer
        dec_out, pred = self.modules.Transformer(
                emb_sentence, tokens_bos, wav_lens, pad_idx=self.hparams.pad_index
            )
        # logits and softmax
        ...
        if stage == sb.Stage.VALID:
            ...
            hyps, _ = self.hparams.valid_search(emb_sentence.detach(), wav_lens)
        elif stage == sb.Stage.TEST:
            hyps, _ = self.hparams.test_search(emb_sentence.detach(), wav_lens)
...
def dataio_prepare(hparams):
    ...
    def sentence_text_pipeline(sentence):
        """Processes the transcriptions to generate proper labels"""
        yield sentence
        tokens_sent = tokenizer_input.sp.encode_as_ids(sentence)
        sentence_tokens = torch.LongTensor([hparams["bos_index"]] + (tokens_sent))
    ...
    tokenizer_input = SentencePiece(
        model_dir=hparams["save_folder"],
        vocab_size=hparams["vocab_size"],
        annotation_train=data_folder + "/train.json",
        annotation_read="sentence",
        annotation_format="json",
        model_type="unigram",
        bos_id=hparams["bos_index"],
        eos_id=hparams["eos_index"],
    ...
    for dataset in ["train"]:
        ...
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            dynamic_items=[audio_pipeline_func, reference_text_pipeline, sentence_text_pipeline],
            output_keys=[
                "id",
                "sig",
                "duration",
    ...
    for dataset in ["valid", "test"]:
        ...
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            json_path=json_path,
            replacements={"data_root": data_folder},
            dynamic_items=[audio_pipeline, reference_text_pipeline, sentence_text_pipeline],
            output_keys=[
                "id",
                "sig",
                "duration",
    ...
    return datasets, tokenizer, tokenizer_input
