...
class ST(sb.core.Brain):
    ...
    def init_optimizers(self):
        ...
        self.wav2vec_optimizer = self.hparams.wav2vec2_opt_class(
            [self.layers_weights]
        )
        self.adam_optimizer = self.hparams.adam_opt_class(self.hparams.model.parameters())

        if self.checkpointer is not None:
            self.checkpointer.add_recoverable("modelopt", self.adam_optimizer)
...
if __name__ == "__main__":
    ...
    st_brain.layers_weights = torch.nn.Parameter(end_init, requires_grad=True)
    torch.save(st_brain.layers_weights, hparams["weights_path"])
    st_brain.layers_weights.to(st_brain.device)
    st_brain.checkpointer.recover_if_possible()
    hf_model.to(st_brain.device)
